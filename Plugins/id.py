...
async def add_edited_msg_count(c: Client, m: Message):
    if not m.from_user:
        return
    uid, cid = str(m.from_user.id), str(m.chat.id)
    key = f"{cid}:TotalEDMsgs:{uid}:{DEV_ID}"
    r.set(key, int(r.get(key) or 0) + 1)
...
"""
    ...
    if text in ("المجموعه", "المجموعة"):
        ...
        if m.chat.photo:
            if m.chat.username:
                ...
                await m.reply_photo(photo, caption=txt)
                try: os.remove(photo)
                except: pass
                return
    ...
    if text in ("مسح المتفاعلين", "تصفير المتفاعلين"):
        ...
        keys = r.keys(f"{DEV_ID}{cid}:TotalMsgs:*")
        for key in keys:
            r.delete(key)
    ...
    if text in ("ترتيبي", "تفاعلي"):
        users_keys = r.keys(f"{DEV_ID}{cid}:TotalMsgs:*")
    ...
    if text in ("القروبات", "توب القروبات"):
        if not is_dev(uid, cid):
            return
        groups_keys = r.keys(f"{DEV_ID}:TotalGroupMsgs:*")
        result = []
        for gkey in groups_keys:
            try:
                chat_id = int(gkey.split("TotalGroupMsgs:")[1])
                msgs    = int(r.get(gkey) or 0)
                title   = (await c.get_chat(chat_id)).title
                result.append({"title": title, "id": chat_id, "msgs": msgs})
            except:
                pass
...
async def add_contact_count(c: Client, m: Message):
    ...
    for member in m.new_chat_members:
        if m.from_user.id != member.id:  # من أضاف شخصاً آخر
            key = f"{m.chat.id}TotalContacts{m.from_user.id}:{DEV_ID}"
            r.set(key, int(r.get(key) or 0) + 1)
...
def _build_top_list(keys, with_name: bool = False) -> list[dict]:
    ...
    for key in keys:
        try:
            ...
            msgs = int(r.get(key) or 0)
            item = {"id": uid, "msgs": msgs}
            if with_name:
                item["name"] = r.get(f"{uid}:bankName") or str(uid)
            result.append(item)
        except:
            pass
    ...
    try:
        ...
        if target.username:
            ...
            async for p in c.get_chat_photos(target.id, limit=1):
                photo = p.file_id
        bio     = (await c.get_chat(target.id)).bio
...
async def _send_id_card(c: Client, m: Message):
    ...
    try:
        ...
        if video:
            ...
            async for chunk in c.stream_media(message=file_id_obj):
                buf.write(chunk)

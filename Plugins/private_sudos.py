...
""",
    ...
    if not r.sismember(f"{DEV_ID}:UsersList", uid):
        ...
        total     = len(r.smembers(f"{DEV_ID}:UsersList"))
        notif_txt = (
...
async def sudos_private_handler(c: Client, m: Message):
    ...
    # ── الاحصائيات ───────────────────────────────────────────────────────
    if text == "الاحصائيات":
        users = len(r.smembers(f"{DEV_ID}:UsersList") or [])
        chats = len(r.smembers(f"enablelist:{DEV_ID}") or [])
        return await m.reply(
            quote=True,
    ...
    if r.get(f"{uid}:pvBroadcast:{DEV_ID}"):
        ...
        users = r.smembers(f"{DEV_ID}:UsersList")
        count = failed = 0
        ...
        for u in users:
            try:
                ...
                failed += 1
        return await rep.edit(f"{k} اذاعة ناجحة {count} | فشل {failed}")

    if r.get(f"{uid}:gpBroadcast:{DEV_ID}"):
        r.delete(f"{uid}:gpBroadcast:{DEV_ID}")
        chats = r.smembers(f"enablelist:{DEV_ID}")
        count = failed = 0
        rep   = await m.reply("جار الاذاعة..")
        for chat in chats:
            try:
